...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		// xml parse step36: 进行xml处理的后置操作
		postProcessXml(root);

		// xml parse step37: 把代理放回原位
	...
	private void parseDefaultElement(Element ele, BeanDefinitionParserDelegate delegate) {
		...
		// xml parse step35: 解析beans标签, 其实这就是递归了
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		...
		if (bdHolder != null) {
			// xml parse step31: BeanDefinitionHolder创建好了之后就开始装饰这个bean
			// (根据逻辑来看, 只有bean标签才会进行装饰, 自定义标签时不会进行装饰的)
			bdHolder = delegate.decorateBeanDefinitionIfRequired(ele, bdHolder);
			try {
				// xml parse step34: 把经过装饰的BeanDefinitionHolder注册到工厂中
				// 注册到最后一个装饰实例.
