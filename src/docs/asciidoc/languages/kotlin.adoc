...
注意: The https://github.com/FasterXML/jackson-module-kotlin[Jackson Kotlin module] is required
...
注意: Keep in mind that Kotlin extensions need to be imported to be used. This means,
...
注意: Other libraries, such as Reactor and Spring Data, also provide Kotlin extensions
...
注意: Libraries such as Reactor or Spring Data provide null-safe APIs to leverage this feature.
...
注意: Generic type arguments, varargs, and array elements nullability are not supported yet,
...
注意: If you use bean validation on classes with properties or a primary constructor
...
注意: This DSL is programmatic, meaning it allows custom registration logic of beans
...
注意: Spring Boot is based on JavaConfig and
...
注意: This DSL is programmatic, meaning that it allows custom registration logic of beans
...
注意: It is also possible to avoid CGLIB proxies for configuration classes by using `@Configuration(proxyBeanMethods = false)`.
...
注意: The Kotlin code samples in Spring Framework documentation do not explicitly specify
...
注意: As of the Kay release train, Spring Data supports Kotlin immutable class instances and
...
注意: Classes with a single constructor have their parameters automatically autowired.
...
注意: If you use Spring Boot, you should probably use
...
注意: If the `@RequestMapping` `method` attribute is not specified, all HTTP methods will
...
注意: If you are using Spring Boot, see
