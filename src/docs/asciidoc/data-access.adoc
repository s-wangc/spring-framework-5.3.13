...
注意: The preceding definition of the `dataSource` bean uses the `<jndi-lookup/>` tag
from the `jee` namespace. For more information see
<<integration.adoc#xsd-schemas-jee, The JEE Schema>>.

注意: If you use JTA, your transaction manager definition should look the same, regardless
...
注意: If the `DataSource` (used by any non-JTA transaction manager) is looked up through
...
注意: Most Spring Framework users choose declarative transaction management. This option has
...
注意: Spring AOP is covered in <<core.adoc#aop, the AOP section>>.
...
the current execution thread. 注意: This does _not_ propagate to newly started threads
within the method.
...
注意: In the preceding example, it is assumed that all your service interfaces are defined
...
注意: Spring Framework uses the `ReactiveAdapterRegistry` to determine whether a method
...
注意: The standard `javax.transaction.Transactional` annotation is also supported as a
...
注意: In proxy mode (which is the default), only external method calls coming in through
...
注意: The default advice mode for processing `@Transactional` annotations is `proxy`,
...
注意: The `proxy-target-class` attribute controls what type of transactional proxies are
...
注意: `@EnableTransactionManagement` and `<tx:annotation-driven/>` look for
...
注意: By default, a participating transaction joins the characteristics of the outer scope,
...
注意: This chapter is not concerned with explaining AOP in any great detail (except as it
...
注意: Prior to continuing, you may want to read <<transaction-declarative-annotations>> and
...
注意: When you use this aspect, you must annotate the implementation class (or the methods
...
注意: As the examples that follow show, using the `TransactionTemplate` absolutely
...
注意: As the examples that follow show, using the `TransactionOperator` absolutely
couples you to Spring's transaction infrastructure and APIs. Whether or not programmatic
...
注意: See the specific coverage of each persistence technology for details on how to
...
注意: The `DataSource` should always be configured as a bean in the Spring IoC container. In
...
注意: The `SQLErrorCodesFactory` is used by default to define `Error` codes and custom exception
...
注意: You should use the `DriverManagerDataSource` and `SimpleDriverDataSource` classes
...
注意: It is rarely desirable to use this class, except when already existing code must be
...
注意: Explicit declarations are necessary if the database you use is not a Spring-supported
...
注意: Only parameters declared as `SqlParameter` and `SqlInOutParameter` are used to
...
注意: Be careful when passing in many values. The JDBC standard does not guarantee that you
...
注意: The `ConnectionFactory` should always be configured as a bean in the Spring IoC
container.
...
注意: `execute(…)` accepts either the SQL query string or a query `Supplier<String>`
...
注意: Usage of select lists is vendor-dependent.
...
注意: R2DBC itself does not support Collection-like values. Nevertheless,
...
注意: Using this class is required if you use a R2DBC client that is not integrated otherwise
...
注意: As of Spring Framework 5.3, Spring requires Hibernate ORM 5.2+ for Spring's
...
注意: Before you continue, we are strongly encourage you to read <<transaction-declarative>>
...
注意: If you want to specifically configure a Hibernate setup, an immediate alternative
...
注意: The `<exclude-unlisted-classes/>` shortcut indicates that no scanning for
...
注意: Although `EntityManagerFactory` instances are thread-safe, `EntityManager` instances are
...
注意: We strongly encourage you to read <<transaction-declarative>>, if you have not
...
注意: As an immediate alternative, Spring's native `HibernateTransactionManager` is capable
...
注意: Although the `marshal()` method accepts a plain object as its first parameter, most
...
注意: Note that XStream is an XML serialization library, not a data binding library.
