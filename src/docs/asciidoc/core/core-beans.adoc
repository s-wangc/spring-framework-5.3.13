...
注意: XML-based metadata is not the only allowed form of configuration metadata.
...
注意: With component scanning in the classpath, Spring generates bean names for unnamed
...
注意: In Spring documentation, "factory bean" refers to a bean that is configured in the
...
注意: The `local` attribute on the `idref` element is no longer supported in the 4.0 beans
...
注意: The `local` attribute on the `ref` element is no longer supported in the 4.0 beans
XSD, since it does not provide value over a regular `bean` reference any more. Change
...
注意: The p-namespace is not as flexible as the standard XML format. For example, the format
...
注意: Due to the XML grammar, the index notation requires the presence of the leading `_`,
...
注意: The `depends-on` attribute can specify both an initialization-time dependency and,
...
注意: The `autowire-candidate` attribute is designed to only affect type-based autowiring.
...
注意: As of Spring 3.0, a thread scope is available but is not registered by default. For
...
注意: The next example uses `SimpleThreadScope`, which is included with Spring but is not
...
注意: When you place `<aop:scoped-proxy/>` within a `<bean>` declaration for a
...
注意: If multiple lifecycle mechanisms are configured for a bean and each mechanism is
...
注意: `ApplicationContext` pre-instantiates all singletons by default. Therefore, it is
...
注意: While the recommended approach for `BeanPostProcessor` registration is through
...
注意: As with ``BeanPostProcessor``s , you typically do not want to configure
...
注意: Specified override values are always literal values. They are not translated into
...
注意: The name provided with the annotation is resolved as a bean name by the
...
注意: When configuring a `PropertySourcesPlaceholderConfigurer` using JavaConfig, the
...
注意: Spring Boot configures by default a `PropertySourcesPlaceholderConfigurer` bean that
...
注意: For brevity, the preceding example could have used the `value` attribute of the
...
注意: You can disable the registration of `AutowiredAnnotationBeanPostProcessor` and
...
注意: You can also disable the default filters by setting `useDefaultFilters=false` on the
...
注意: `@Scope` annotations are only introspected on the concrete bean class (for annotated
...
注意: To provide a custom strategy for scope resolution rather than relying on the
...
注意: As with most annotation-based alternatives, keep in mind that the annotation metadata is
...
注意: Your existing `@ComponentScan` or `<context:component-scan/>` directives must remain
...
注意: When working with this mode in your IDE, the `spring-context-indexer` must be
...
注意: In contrast to `@Component`, the JSR-330 `@Named` and the JSR-250 `ManagedBean`
...
注意: Remember that `@Configuration` classes are <<beans-meta-annotations, meta-annotated>>
...
注意: This method of declaring inter-bean dependencies works only when the `@Bean` method
...
注意: The behavior could be different according to the scope of your bean. We are talking
...
注意: In `system-test-config.xml` file, the `AppConfig` `<bean/>` does not declare an `id`
...
注意: As mentioned earlier, with `@Bean` methods, you typically choose to use programmatic
...
注意: You cannot mix the `&` and `|` operators without using parentheses. For example,
...
注意: These default property sources are present for `StandardEnvironment`, for use in standalone
...
注意: The `@PropertySource` annotation is repeatable, according to Java 8 conventions.
...
注意: Because Spring's `MessageSource` is based on Java's `ResourceBundle`, it does not merge
...
注意: As an alternative to `ResourceBundleMessageSource`, Spring provides a
...
注意: Spring's eventing mechanism is designed for simple communication between Spring beans
...
注意: This feature is not supported for
...
注意: Developers should not use the `"spring.*"` namespace when creating custom startup steps.
...
注意: Such RAR deployment units are usually self-contained. They do not expose components
