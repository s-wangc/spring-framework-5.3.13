...
注意: The `Resource` abstraction does not replace functionality. It wraps it where
...
注意: To load one or more `Resource` objects for a resource path that contains wildcards
...
注意: The wildcard classpath relies on the `getResources()` method of the underlying
