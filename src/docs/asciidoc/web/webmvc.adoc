...
注意: In addition to using the ServletContext API directly, you can also extend
...
注意: Spring Boot follows a different initialization sequence. Rather than hooking into
...
注意: Spring Boot relies on the MVC Java configuration to configure Spring MVC and
...
注意: With Servlet 3.0 multipart parsing, you may also declare `javax.servlet.http.Part`
...
注意: `@JsonView` allows an array of view classes, but you can specify only one per
...
注意: When a name is not explicitly specified, a default name is chosen based on the `Object`
...
注意: As of 5.1, `ServletUriComponentsBuilder` ignores information from the `Forwarded` and
...
注意: Controller method signatures are limited in their design when they are supposed to be usable for
...
注意: As of 5.1, `MvcUriComponentsBuilder` ignores information from the `Forwarded` and
`X-Forwarded-*` headers, which specify the client-originated address. Consider using the
<<filters-forwarded-headers, ForwardedHeaderFilter>> to extract and use or to discard
such headers.
...
注意: See <<core.adoc#format-FormatterRegistrar-SPI, the `FormatterRegistrar` SPI>>
...
注意: Enabling indentation with Jackson XML support requires
