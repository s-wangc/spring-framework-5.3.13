...
注意: A request to overcome the preceding limitation in the Java WebSocket API has been
...
注意: For client-side WebSocket configuration, you should use `WebSocketContainerFactoryBean`
...
注意: During initial development, do enable the SockJS client `devel` mode that prevents
...
注意: When using STOMP over WebSocket and SockJS, if the STOMP client and server negotiate
...
注意: As a result, network I/O failures can occur because a client has disconnected, which
...
注意: SockJS uses JSON formatted arrays for messages. By default, Jackson 2 is used and needs
...
注意: For the built-in simple broker, the `/topic` and `/queue` prefixes do not have any special
...
注意: Applications can also use dot-separated (rather than slash-separated) destinations.
...
注意: Add `io.projectreactor.netty:reactor-netty` and `io.netty:netty-all`
...
注意: The STOMP broker relay always sets the `login` and `passcode` headers on every `CONNECT`
...
注意: The preceding limitations are for browser-based clients and do not apply to the
...
注意: While user destinations generally imply an authenticated user, it is not strictly required.
...
注意: When you use user destinations with an external message broker, you should check the broker
...
注意: When you use a full-featured broker, the STOMP "`broker relay`" automatically reconnects the
...
注意: When you use `WebSocketStompClient` for performance tests to simulate thousands
