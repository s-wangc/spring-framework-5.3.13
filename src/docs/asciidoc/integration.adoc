...
注意: This is not portable in a Java EE environment. It is mainly intended for non-EE
...
注意: The above is slightly simplified in that JAX-WS requires endpoint interfaces
...
注意: The `servicePort` property has been omitted (it defaults to 0). This means that an
...
注意: The preceding example does not show a flexible kind of security infrastructure. For
...
注意: For EJB 3 Session Beans, you can effectively use a `JndiObjectFactoryBean` /
...
注意: While `SimpleMessageListenerContainer` does not allow for participation in externally
...
注意: Like its sibling (<<jms-mdp-simple, `SimpleMessageListenerContainer`>>),
...
注意: Spring also supports annotated-listener endpoints through the use of the `@JmsListener`
...
注意: JCA-based message endpoint management is very analogous to EJB 2.1 Message-Driven Beans.
...
| A reference to the `MessageConverter` strategy for converting JMS Messages to listener
  method arguments. 默认值为`SimpleMessageConverter`.
...
注意: `MBeanExporter` is a `Lifecycle` bean (see <<core.adoc#beans-factory-lifecycle-processor,
...
注意: A `ManagedResource`-annotated bean must be public, as must the methods exposing
...
注意: This section is really only relevant to Spring-managed beans that have
...
注意: The mail code is a crosscutting concern and could well be a candidate for
...
注意: The default advice mode for processing `@Async` annotations is `proxy` which allows
...
注意: There is no direct XML equivalent for `@Async`, since such methods should be designed
...
注意: By using the `name` and `group` properties, you can modify the name and the group
...
注意: By default, jobs will run in a concurrent fashion.
...
注意: All the other caches that do not contain the value are also updated, even though
...
注意: The `key` and `keyGenerator` parameters are mutually exclusive and an operation
...
注意: This is an optional feature, and your favorite cache library may not support it.
...
注意: The default advice mode for processing caching annotations is `proxy`, which allows
for interception of calls through the proxy only. Local calls within the same class
cannot get intercepted that way. For a more advanced mode of interception, consider
switching to `aspectj` mode in combination with compile-time or load-time weaving.

注意: For more detail about advanced customizations (using Java configuration) that are
...
注意: `<cache:annotation-driven/>` looks for `@Cacheable/@CachePut/@CacheEvict/@Caching`
...
注意: In proxy mode (the default), only external method calls coming in through the
...
注意: For all referenced classes, Spring tries to locate a bean with the given type.
...
注意: Depending on your use case, the choice is basically yours. You can even mix and
...
注意: Ehcache 3.x is fully JSR-107 compliant and no dedicated support is required for it.
