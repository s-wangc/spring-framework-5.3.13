...
注意: `@ContextConfiguration` provides support for inheriting resource locations or
...
注意: `@ActiveProfiles` provides support for inheriting active bean definition profiles
...
注意: As of Spring Framework 5.2, `@TestConstructor` is only supported in conjunction
...
注意: The use of `@NestedTestConfiguration` typically only makes sense in conjunction
...
注意: These events are only published if the `ApplicationContext` has already been loaded.
...
注意: As of Spring Framework 5.3, test configuration may also be inherited from enclosing
...
注意: You can use `@ActiveProfiles` with any implementation of the `SmartContextLoader`
...
注意: If you use `@DirtiesContext` in a test whose context is configured as part of a
...
注意: Although field injection is discouraged in production code, field injection is
...
注意: Method-level `@Sql` declarations override class-level declarations by default. As
...
注意: MockMvc works with templating technologies that do not rely on a Servlet Container
...
注意: This is a simple example of using `MockMvcWebClientBuilder`. For advanced usage,
...
注意: The default context path is `""`. Alternatively, we can specify the context path,
...
注意: Despite being a part of https://docs.seleniumhq.org/[Selenium], WebDriver does not
...
注意: This is a simple example of using `MockMvcHtmlUnitDriverBuilder`. For more advanced
...
注意: We use an assertion in the closure so that we can determine where things went wrong
